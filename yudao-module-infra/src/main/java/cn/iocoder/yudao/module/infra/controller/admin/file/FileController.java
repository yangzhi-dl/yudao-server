...
public class FileController {
    ...
    @Autowired
    private List<FileAccessChecker> fileAccessCheckers;
    ...
    @Operation(summary = "上传文件", description = "模式二：后端上传文件")
    @Parameter(name = "file", description = "文件附件", required = true,
            schema = @Schema(type = "string", format = "binary"))
    public CommonResult<FileDO> uploadFileToData(@Valid FileUploadReqVO uploadReqVO) throws Exception {
        MultipartFile file = uploadReqVO.getFile();
        byte[] content = IoUtil.readBytes(file.getInputStream());
        return success(fileService.createFileToData(content, file.getOriginalFilename(),
                uploadReqVO.getDirectory(), file.getContentType()));
    }
    ...
    @GetMapping("/user/get")
    @Operation(summary = "用户获取文件")
    @Parameter(name = "id", description = "编号", required = true)
    public CommonResult<FileRespVO> getUserFile(@RequestParam("id") Long id) {
        FileDO file = fileService.getFile(id);
        String loginUserId = String.valueOf(getLoginUserId());
        boolean isCreator = Objects.equals(loginUserId, file.getCreator());
        if (!isCreator && fileAccessCheckers.stream().noneMatch(checker -> checker.canAccess(id))) {
            return error(new ErrorCode(403, "无该文件资源权限！"));
        }
        file.setUrl(fileService.presignGetUrl(file.getPath(), 60));
