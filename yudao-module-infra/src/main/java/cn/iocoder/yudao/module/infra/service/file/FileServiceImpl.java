...
public class FileServiceImpl implements FileService {
    ...
    public PageResult<FileDO> getFilePage(FilePageReqVO pageReqVO) {
        ...
        List<FileDO> list = fileDOPageResult.getList();
        list.forEach(item -> item.setUrl(this.presignGetUrl(item.getPath(), 60)));
        return fileDOPageResult;
    }
    ...
    @Override
    @SneakyThrows
    public FileDO createFileToData(@NotEmpty(message = "文件内容不能为空") byte[] content, String name, String directory, String type) {
        // 1.1 处理 name 的合法性，禁止携带目录路径
        name = FilePathUtils.validateFileName(name);
    ...
    @Override
    @SneakyThrows
    public FileDO createFile(@NotEmpty(message = "文件内容不能为空") byte[] content) {
        String name = DigestUtil.sha256Hex(content);
        String type = FileTypeUtils.getMineType(content, name);
        name = name + FileTypeUtils.getExtension(type);
    ...
    @Override
    public List<FileDO> getFileByIds(List<Long> ids) {
