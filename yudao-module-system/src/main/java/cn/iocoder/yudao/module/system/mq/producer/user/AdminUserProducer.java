...
public class AdminUserProducer {
    ...
    /**
     * 发布用户新增或编辑事件，供可选业务模块初始化用户配置。
     *
     * @param userId 用户编号
     */
    public void sendUserCreatedOrUpdatedMessage(Long userId) {
        applicationContext.publishEvent(new AdminUserCreatedOrUpdatedMessage().setUserId(userId));
    }
     ...
     */
    public void sendUserProfileUpdateMessage(Long userId, String nickname, Long avatar) {
        applicationContext.publishEvent(new AdminUserProfileUpdateMessage()
                .setUserId(userId).setNickname(nickname).setAvatar(avatar != null ? avatar.toString() : null));
    }
