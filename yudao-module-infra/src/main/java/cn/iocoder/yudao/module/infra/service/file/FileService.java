...
public interface FileService {
     ...
     * @return 文件路径
     */
    FileDO createFileToData(@NotEmpty(message = "文件内容不能为空") byte[] content,
                      String name, String directory, String type);
    ...
    default String presignGetUrl(Long id, Integer expirationSeconds) {
        try {
            FileDO file = getFile(id);
            return presignGetUrl(file.getPath(), expirationSeconds);
        } catch (Exception e) {
            return null;
        }
    }

    default String presignGetUrl(FileDO file, Integer expirationSeconds) {
        try {
            return presignGetUrl(file.getPath(), expirationSeconds);
        } catch (Exception e) {
            return null;
    ...
    List<FileDO> getFileByIds(List<Long> ids);
